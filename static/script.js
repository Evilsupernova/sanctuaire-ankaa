// V10 front adapté : titre 1 ligne, outils empilés, bouton ☥ caché après ouverture
document.addEventListener('DOMContentLoaded', () => {
  ...
  // déverrouillage audio iOS 1er tap
  (function unlock(){
    const a = [document.getElementById('musique-sacree'), document.getElementById('tts-player'),
               document.getElementById('s-click'), document.getElementById('s-open'),
               document.getElementById('s-close'), document.getElementById('s-mode')];
    function arm(){
      a.forEach(x=>{ if(!x) return; x.muted=true; const p=x.play(); if(p&&p.finally) p.finally(()=>{x.pause();x.muted=false;}); });
  ...
  if(btnMode) btnMode.disabled = false; // on peut toujours re-ouvrir le choix
  ...
  btnOpen?.addEventListener('click', ()=>{
    ...
    // le bouton d’ouverture disparaît après ouverture
