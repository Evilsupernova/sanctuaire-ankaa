# app.py — Sanctuaire Ankaa (Mobile V10 stable)
# - Invocation : VOIX FEMME (dialogue)
# - Souffle    : VOIX HOMME (fragments du dataset)
# - edge-tts sans SSML (compat max) + pitch/rate valides
...
def greet(): return "Salut, frère 🌙. Je t’écoute — de quoi veux-tu parler ?"
def rag_answer(u):
    src=retrieve(u, k=3)
...
def answer(user, mode):
    ...
    if Llama is not None and os.getenv("ANKAA_USE_LOCAL_LLM")=="1":
        try:
            ...
            prompt=("Réponds en français, clair et humain, sans balises ni technicalités.\n"
                    f"Question: {user}\nRéponse:")
            res=llm.create_completion(prompt=prompt, max_tokens=512, temperature=0.8, top_p=0.95)
            txt=(res.get("choices",[{}])[0].get("text","") or "").strip()
        except: txt=None
    if not txt:
        base=rag_answer(user)
        toks=[t for t in _tok(user) if t not in STOP_FR]
...
async def _tts_async(text, voice, rate, pitch, out):
    if edge_tts is None: return "disabled"
    # Pas de SSML ici -> compat max (Render)
...
def do_tts(text, mode, is_souffle, out_file: Path):
    ...
    try:
        if is_souffle:
            ...
            rate, pitch = "-2%", "+0%"   # ← valeurs valides
        else:
